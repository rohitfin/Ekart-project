...
export class GeneralService {
  ...
  header = new HttpHeaders({'my-headers': 'hello-world'})
  
  constructor(
    private http: HttpClient
  ) { }

  //https://stackblitz.com/run?file=src%2Findex.html
  ...
  getDate(date?: any){
    ...
    var currentDate = (day.length == 1 ? ('0'+day): day) +'-'+(month.length == 1 ? ('0'+month): month)+'-'+year+'T'+(hours.length == 1 ? ('0'+hours): hours)+':'+(minutes.length == 1 ? ('0'+minutes): minutes)+':'+(seconds.length == 1 ? ('0'+seconds): seconds);
    return currentDate;
  }
  
  ipAddress: any;
  public getClientIPAddress() {
    return this.http.get("http://api.ipify.org/?format=json", {responseType: 'json'});
  }



  getData(url: string): Observable<any> {
    return this.http.get<any>(url, { headers: this.header }).pipe(map((res)=>{
      let resp: any = [];
      for(let key in res){
        console.log(res.hasOwnProperty(key));
        resp.push({...res[key], id: key});
      }
      return resp;
    }));
  }

  addorUpdateData(url: string, updateUrl:string, body: any, id: number): Observable<any> {
    if (id) {
      return this.http.put<any>(updateUrl, body, { headers: this.header });
    } else {
      Object.keys(body).forEach((key) => {
        body[key] = (body[key] == "" || body[key] == "0" || body[key] == "null") ? null : body[key];
      })
      body["createdDate"] = this.getDate();
      body["createdAdress"] = this.getDate();
